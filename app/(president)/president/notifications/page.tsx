...
export default function PresidentNotificationsPage() {
  const [items, setItems] = useState<PresidentNotification[]>([]);
    ...
    const fetchNotifications = async () => {
      try {
        const res = await fetch(
          `/api/president/notifications?limit=100&tab=${activeTab}`,
          {
            cache: "no-store",
          },
        );
        ...
        if (!mounted) return;
        setItems((body?.notifications || []) as PresidentNotification[]);
  ...
  const handleMarkAllRead = async () => {
    const res = await fetch("/api/president/notifications", {
      method: "PATCH",
    ...
    setUnreadCount(0);
    window.dispatchEvent(new CustomEvent("eventgate:notifications-updated"));
  };
  ...
  const updateNotification = async (
    ...
    setUpdatingId(id);
    try {
      const res = await fetch(`/api/president/notifications/${id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  const grouped = items.reduce<Record<string, PresidentNotification[]>>(
    (acc, item) => {
