...
export function SecretaryHeader({ userEmail }: { userEmail: string }) {
  ...
  React.useEffect(() => {
    ...
    const fetchNotifications = async () => {
      try {
        const response = await fetch("/api/notifications/secretary?limit=10", {
          cache: "no-store",
        });
    ...
    const handleNotificationsUpdated = (event: Event) => {
      ...
      if (
        ...
        customEvent.detail.audience === "secretary"
      ) {
        fetchNotifications();
      }
    };
      ...
      <div className="w-full text-gray-900 bg-white border-b border-gray-200">
        <div className="container mx-auto flex items-center justify-between gap-4 px-4 py-2 lg:px-8">
          ...
          <nav className="hidden md:flex flex-1 items-center justify-center gap-2">
            ...
            <Link
              href="/secretary/approved"
              className={`relative inline-flex items-center h-9 px-3 text-sm font-medium transition-colors ${
                isActive(pathname, "/secretary/approved")
                  ? "text-[var(--aau-blue)]"
                  : "text-gray-700 hover:text-[var(--aau-blue)]"
              }`}
            >
          ...
          <div className="flex items-center gap-2 justify-end">
            <Link href="/secretary/notifications" className="block">
              <Button
                variant="secondary"
            ...
            <Dialog open={menuOpen} onOpenChange={setMenuOpen}>
              ...
              <DialogContent className="fixed right-0 top-0 left-auto bottom-0 h-dvh w-[20rem] max-w-[calc(100%-3rem)] translate-x-0 translate-y-0 rounded-none p-0 sm:max-w-[20rem]">
                <div className="flex h-full flex-col">
                  ...
                  <nav aria-label="Secretary navigation" className="flex-1 p-2">
                    <Link
                      href="/secretary"
                    ...
                    >
                      <Button
                        variant={
                          isActive(pathname, "/secretary") ? "default" : "ghost"
                        }
                    ...
                    >
                      <Button
                        variant={
                          isActive(pathname, "/secretary/notifications")
                            ? "default"
                            : "ghost"
                        }
                        className="h-10 w-full justify-start rounded-none"
